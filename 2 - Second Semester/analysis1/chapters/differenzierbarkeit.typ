...
#concept-block[
  ...
  #Kor Sei $f: D -> E$ bijektiv, $x_0$ HP, $f$ diff. in $x_0$ mit $f'(x_0) != 0$, und $f^(-1)$ stetig in $y_0 = f(x_0)$. Dann ist $y_0$ HP von $E$, $f^(-1)$ diff. in $y_0$ und $ (f^(-1))'(y_0) = 1/(f'(f^(-1)(y_0))) $
  ...
  *Rechenregeln:*
  ...
  2. $(f g)' = f'g + g'f$
  3. $(f/g)' = (f'g-f g')/g^2$
  4. $(g compose f)' = g'(f) dot f'$
  5. $(a^f)' = ln(a) dot a^f dot f'$
  6. $(f^g)' = f^g [ln(f) dot g]'$.
]
...
#concept-block[
  ...
  *Höhere Ableitungsregeln:*
  ...
  #tbox[
    #Satz Sei $sum c_k (x-x_0)^k$ eine Potenzreihe mit Konvergenzradius $p > 0$.
    Dann ist $f(x) = sum_(k>=0) c_k (x-x_0)^k$ auf $]x_0-p, x_0+p[$ differenzierbar
    und
    $ f'(x) = sum_(k>=1) k c_k (x-x_0)^(k-1). $
    Die abgeleitete Reihe hat denselben Konvergenzradius $p$. Iteriert folgt:
    $f$ ist glatt auf $]x_0-p, x_0+p[$ und stimmt dort mit ihrer Taylorreihe überein,
    d.h. $c_k = f^((k))(x_0) \/ k!$.
  ]
]
